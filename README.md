...
[ ] Убедитесь, что все пароли хэшируются с использованием соответствующего шифра, вроде bcrypt. Никогда не пишите свой собственный шифр и корректно инициализируйте шифр из случайных данных.
...
[ ] Создайте лимиты размерам и структурам запросов для данных, предоставляемых пользователями.

[ ] Снизьте риск хакерской атаки, минимизировав последствия Distributed Denial of Service (DDOS), используйте для этого глобальный кэширующий прокси-сервер вроде CloudFlare. Его можно включить, если вы подверглись DDOS-атаке или использовать его для поиска DNS.
...
[ ] Используйте CSP, не допуская unsafe-* бэкдоры. Настраивается это через боль, но оно того стоит.
...
[ ] Используйте токены CSRF во всех формах и новый заголовок ответа SameSite Cookie, который раз и навсегда чинит CSRF для всех новых браузеров.
...
[ ] Валидируйте информацию на клиенте, чтобы пользователь получал быстрый фидбек, но всегда делайте повторную валидацию.
...
[ ] Ограничьте исходящий IP-трафик и трафик портов для минимизации APT (Advansed Persistent Threat) и бот-атак.
...
[ ] Используйте централизованную регистрацию для всех служб. Вам никогда не понадобится SSH для доступа к журналам или их извлечения.

[ ] Не используйте протокол SSH в службах, только если необходима единоразовая диагностика. Регулярное использование SSH чаще всего означает, что вы не автоматизировали важную задачу.

[ ] Не оставляйте порт 22 постоянно открытым в любых группах служб AWS.

[ ] Создавайте немутабельные хосты, вместо серверов с длительным сроком службы, которые нужно модифицировать и обновлять. (Immutable Infrastructure Can Be More Secure).

[ ] Используйте Intrusion Detection System (Систему обнаружения вторжений) для минимизации APT.

## Функционирование

[ ] Выключайте неиспользуемые службы и серверы. Самый безопасный сервер — отключенный сервер. С помощью инструментов вроде PowerDown, можно такому процессу задать расписание. 

## Тестирование

[ ] Проверьте свой дизайн и реализацию.
